...
- Comments citing a decision use `[D:key]` from docs/DECISIONS.md (a keyed
...
- Bash only via `sh -c "..."` inside weir, or as a full fallback when
  ...
  dev/NOTES-agent.md under `## fallbacks` explaining the forcing gap.
- Scripts abandoned after 3 failed check iterations: append the script
  and the final error verbatim to dev/NOTES-agent.md under `## stranded`.
