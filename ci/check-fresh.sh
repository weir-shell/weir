#!/usr/bin/env bash
# The single freshness gate [D:masking-mechanized] — the binary's build
...
# subprocesses it), so the guard has one implementation that cannot
...
# stamp cannot arise there, and keying a hard-fail on the ambient
# `CI` var (set in many non-release contexts) only ever false-positived
# on developers. A release job that wants the strictness sets
