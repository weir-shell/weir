...
| regex-pattern | 2026-07-22 | the bespoke `Regex "lit" binder` match form — literal-only, check-time compiled (shared literal-keyed cache), arity-typed binders, raw literal, refutable; Str.isMatch/rmatch carry computed patterns; NOT user active patterns (door stays closed) | divergences.md row regex-pattern; SEMANTICS: branching; TRANSCRIPTION addendum |
