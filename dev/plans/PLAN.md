# Typed F# Shell — Exploration Plan

Status: EXECUTED (landed 2026-07-11).

Scope: learning spikes, not a product build. Each spike is a self-contained session with a concrete deliverable and a kill/continue question. Ordered by risk. Work spike by spike.

## Setup (one session)

- dotnet F# console project, Expecto test project, FParsec dependency.
...
- Also: `_.Field` lambda shorthand (parser-level desugar to `fun x -> x.Field`; rides the existing lambda check rule, only valid with ≥1 field like F# 8).
...
If Spike 1 takes >3x estimate or you can't confidently review the checker, stop and do the type-theory reading (Dunfield & Krishnaswami) before continuing — pushing through with code you can't verify defeats the purpose.
