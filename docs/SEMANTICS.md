...
author's decision, nesting included; the concurrency review measured
12×12 nested running 144-wide before this). Not `ProcessorCount`: the
ceiling exists for RESOURCE protection (an unbounded fan-out over 10k
items is a fork bomb with good manners), not CPU sizing. `Seq.pmapWith n` / `Seq.piterWith n` set it
