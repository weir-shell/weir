...
- 2026-07-30 [PROPOSED] [DESIGN-yaml-templates.md](DESIGN-yaml-templates.md) — PROPOSED design, awaiting bless: YAML templates — the `yaml` district (checked block literal, typed splices, the `for` specialization), records-as-templates via `to yaml`, `from yaml T` tree law; one OWNED subset parser (no YamlDotNet — the config-format spike's receipt); 2-3 session arc, depends on PLAN-for-do
- 2026-07-30 [PROPOSED] [PLAN-for-do.md](PLAN-for-do.md) — PROPOSED mini-plan, awaiting bless: `for`/`do` as weir's GENERAL iteration form (statement → Seq.iter desugar, comprehension `[for x in xs -> e]` → Seq.map; two reserved words; the pipelines-transform-for-effects law) — prerequisite of the yaml district's `for`
